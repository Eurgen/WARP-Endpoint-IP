...
endpointyx() {
	...
	wget https://raw.githubusercontent.com/Eurgen/WARP-Endpoint-IP/main/files/warp-linux-$(archAffix) -O warp
...
endpoint4() {
	...
	iplist=100
	while true; do
		temp[$n]=$(echo 8.47.69.0.$(($RANDOM % 256)))
		n=$(($n + 1))
		if [ $n -ge $iplist ]; then
			break
		fi
		temp[$n]=$(echo 8.47.70.0.$(($RANDOM % 256)))
		n=$(($n + 1))
		if [ $n -ge $iplist ]; then
			break
		fi
		temp[$n]=$(echo 8.47.71.0.$(($RANDOM % 256)))
		n=$(($n + 1))
		if [ $n -ge $iplist ]; then
			break
		fi
